...
TEST(UniverseTest, ParticleScatter) {
    ...
    EXPECT_NE(universe.getState(1).pos.x, state1.pos.x);
    EXPECT_DOUBLE_EQ(universe.getState(1).pos.y, state1.pos.y);

    for(; t < 70; t += dT) universe.advance(dT); // Particles have interacted
    EXPECT_NE(universe.getState(1).pos.y, state1.pos.y);
}

TEST(UniverseTest, Bounds) {
    Universe universe({ 10, 10, 1, 0 }, { ParticleType(1, 1, 0, 0, 0) }); // Non-interacting particle
    ...
    ASSERT_GT(universe.getState(0).pos.x, -5);
    ASSERT_LT(universe.getState(0).pos.x, 15);
    ASSERT_GT(universe.getState(1).pos.y, -5);
    ASSERT_LT(universe.getState(1).pos.y, 15);
}
...
TEST(UniverseTest, Gravity) {
    ...
    ASSERT_GT(universe.getState(0).pos.y, state.pos.y);
}
